"""Ingest the verified IBGE municipality directory into immutable RAW storage."""
...
def main() -> None:
    ...
    source = load_source_registry(Path("config/sources.yml"))["municipalities"]
    if not source.enabled or not source.resource_url:
        raise RuntimeError("The municipalities source is not ready for ingestion")
